...
# get 4 points on each circle to approximate bounding circle
def from_circle_set(circles):
    points = []
    for c in circles:
        if c.is_placed():
            points.append((c.x + c.r, c.y))
            points.append((c.x , c.y + c.r))
            points.append((c.x - c.r, c.y))
            points.append((c.x , c.y - c.r))
    c = make_circle(points)
    return Circle(c[2], c[0], c[1])
